#!/usr/bin/env bash
# Cartola Clara — estado desplegado en un comando. Solo curl + grep.
set -uo pipefail
APP="https://cartola-clara.vercel.app"
OUT="cartola-clara-state.md"
CB="cb=$(date +%s)"
{
echo "# Cartola Clara — estado desplegado"
echo
...
RESP=$(curl -fsSL -m 90 -X POST "$APP/api/verdicto" -H "Content-Type: application/json" \
  -d '{"productId":"cartola","texto":"sueldo 450.000, arriendo 180.000, deudas tarjeta 90.000, delivery como 8 veces al mes..."}' 2>/dev/null)
echo '```'
